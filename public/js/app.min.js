...
angular.module("DepartamentosCtrl", []).controller("DepartamentosCtrl", [
    "$scope",
    "$rootScope",
    "$http",
    "$injector",
    "$mdDialog",
    function($scope, $rootScope, $http, $injector, $mdDialog) {
        console.info("DepartamentosCtrl");
        var Ctrl = $scope;
        var Rs = $rootScope;

        Ctrl.Salir = $mdDialog.cancel;
		
        Ctrl.DepartamentosCRUD = $injector.get("CRUD").config({
        ...
        Ctrl.getDepartamentos = () => {
            Ctrl.DepartamentosCRUD.get().then(() => {
                //Ctrl.editarDepartamento(Ctrl.DepartamentosCRUD.rows[0]);
            });
        };

        Ctrl.getDepartamentos();

        Ctrl.nuevaDepartamento = () => {
            Ctrl.DepartamentosCRUD.dialog({
                Flex: 10,
				Title: 'Crear Departamento',
				
				Confirm: { Text: 'Crear Departamento' },
            }).then(r => {
                if (!r) return;
                Ctrl.DepartamentosCRUD.add(r);
            });
        };

        Ctrl.editarDepartamento = (D) => {
			Ctrl.DepartamentosCRUD.dialog(D, {
				title: 'Editar Departamento' + D.departamento
			}).then(r => {
				if(r == 'DELETE') return Ctrl.DepartamentosCRUD.delete(D);
				Ctrl.DepartamentosCRUD.update(r).then(() => {
					Rs.showToast('Departamento actualizada');
				});
			});
		}

		Ctrl.eliminarDepartamento = (D) => {
			Rs.confirmDelete({
				Title: '¿Eliminar Departamento #'+D.id+'?',
			}).then(d => {
				if(!d) return;
				Ctrl.DepartamentosCRUD.delete(D);
			});
        }

		Ctrl.abrirDepartamento = (D) => {
            console.log(D);
			$mdDialog.show({
				templateUrl: 'Frag/GestionDepartamento.DepartamentoDiag',
				controller: 'DepartamentoDiagCtrl',
				locals: { Departamento: D },
				fullscreen: false,
			});
        }
