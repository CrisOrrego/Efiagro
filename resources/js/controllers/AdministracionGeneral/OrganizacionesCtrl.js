angular.module("OrganizacionesCtrl", [])
    .controller("OrganizacionesCtrl", ["$scope", "$rootScope", "$http", "$injector", "$mdDialog", "Upload", //DEV ANGÉLICA --> 
        function ($scope, $rootScope, $http, $injector, $mdDialog, Upload) {
            ...
            loadMunicipios = (valorDepartamento, col_municipio) => {
                ...
                $http.post('api/lista/obtener', {
                    lista: 'Municipios',
                    Op1: valorDepartamento
                }).then((r) => {
            ...
            Ctrl.nuevaOrganizacion = () => { //Esta es una función que me crea automaticamente la modal y lleva la informacion a la BD desde la modal de CRUD
                ...
                Ctrl.OrganizacionesCRUD.dialog({}, {
                    ...
                    // if (!r) return;
                    Ctrl.OrganizacionesCRUD.add(org)
                        .then(() => {
                            Rs.showToast('Oganización creada')
                        });
                });
            };
